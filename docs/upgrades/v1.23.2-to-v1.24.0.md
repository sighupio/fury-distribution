...
This guide describes the steps to follow to upgrade the Kubernetes Fury Distribution (KFD) from v1.23.2 to v1.24.0.
...
> ⚠️ some downtime of the Monitoring stack is expected during this process.
...
> ⚠️ some downtime of the Logging stack is expected during this process.
...
> 💡 we recommend leaving the ElasticSearch/Kibana stack up for a breif period (like 30 days) and then proceed to delete it.
...
> ⚠️ notice that the `http.send` OPA built-in is disabled. [Read here for more details](https://open-policy-agent.github.io/gatekeeper/website/docs/externaldata#motivation).
...
> 💡 Pomerium is at the same previously included version and Dex has been updated for compatibility with Kubernetes 1.24.x, there are no breaking changes.
...
🎉 **CONGRATULATIONS** you have now successfully updated to KFD 1.24.0 🎉
