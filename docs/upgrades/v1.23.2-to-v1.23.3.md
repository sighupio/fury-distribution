...
Alert Manager configuration now expects 3 new secrets `infra-slack-webhook`, `k8s-slack-webhook` and `healthchecks-webhook` in the `monitoring` namespace with the endpoints where to send the alerts in the `url` key. We recommend you add them to your Kustomize base. Example commands to create the secrets:
...
Wait a minute and check that you can see metrics in Grafana, both old and new, check that all Prometheus Targets are up, and that Alert Manager is working as expected.
...
Since this upgrade is a major version one, there are some manual steps involving breaking changes that you need to do before applying the project:
...
# FIXME: not sure about this one. Delete cert-manager deployment
# kubectl delete -n cert-manager deployments.apps cert-manager cert-manager-webhook
# finally
kustomize build <your-project-path> | kubectl apply -f - --server-side --force-conflicts
```
...
# check the logs to verify that everything went OK
velero backup logs test-upgrade -n kube-system
```
...
You can try to deploy a pod that is not compliant with the rules deployed in the cluster and also check Gatekeeper Policy Manager for new violations to the constraints.
